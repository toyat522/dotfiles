...
public:
    PointCloudFilter() : Node("point_cloud_filter")
    {
        ...
        this->declare_parameter<double>("half_fov", 0);
        ...
        m_half_fov = this->get_parameter("half_fov").as_double();
    }
...
private:
    ...
    {
        double half_fov = m_half_fov * M_PI / 180.0;
        ...
        for (const auto &point : in_cloud_ptr->points)
        {
            ...
            // If in range thresholds, intensity thresholds, and fov, then keep point
