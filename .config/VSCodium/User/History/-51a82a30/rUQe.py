...
class Controller(Node):
    ...
    def __init__(self):
        ...
        pid_omega = PID((1, 0.0, 0) if self.in_sim else (0.0, 0.00, 0), self.get_omega_feedback)#0.8, 0.04, 0
        ...
        pid_vx = PID((0.4, 0.0, 0) if self.in_sim else (0, 0, 0), self.get_x_velocity_world_feedback, None, BLIND_LINEAR)
        pid_vy = PID((0.4, 0.0, 0) if self.in_sim else (0, 0, 0), self.get_y_velocity_world_feedback, None, BLIND_LINEAR)
        local_pid_vx = PID((0.4, 0.0, 0) if self.in_sim else (1, 0, 0), self.get_x_velocity_local_feedback, None, BLIND_LINEAR)
        local_pid_vy = PID((0.4, 0.0, 0) if self.in_sim else (0.0, 0, 0), self.get_y_velocity_local_feedback, None, BLIND_LINEAR)
    ...
    def update_odometry(self, msg):
        ...
        self.get_logger().info(f"setpoint: {self.x_control_handlers[self.chosen_linear_control_mode].feedback}")
        self.get_logger().info(f"feedback: {self.x_control_handlers[self.chosen_linear_control_mode].feedback}")
